...
AutoLayout.prototype.layoutProcess = function(xmlStr, callback) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    moddle.fromXML(xmlStr, function(error, moddleWithoutDi) {
      ...
      var root = moddleWithoutDi.get('rootElements').find(el => /*el.$type === 'bpmn:Collaboration'*/ el.$type === 'bpmn:Process' );
/*
      if(root.$type === 'bpmn:Collaboration') {

        if(root.participants) {

          process = moddle.create('bpmndi:BPMNShape', {
            id: root.id,
            bpmnElement: root.participants[0],
            isHorizontal: true,
            bounds: {
              width: 100,
              height: 100
          ...
          root = root.participants[0].processRef;

          root.flowElements.push(process);
...
function getDataOutputAssociation(source, flowElements) {
  return flowElements ? flowElements.filter(e => {
    return is(e.$type, 'bpmn:DataOutputAssociation') && e.get('$parent').id === source.id;
  }) : [];
}
